...
export default class FriendsList extends React.Component {
	constructor( props ) {
		super( props );
    // ^this has to do with inheritance, takes in whatever props are being passed from a parent
		this.state = {
			  searchText: "",
			  orderBy: "name",
			  order: "ascending"
  ...
  //would have to write two handlers, wouldn't need field to pass in, just the event
	handleChange( field, event ) {
		this.setState({
      [ field ]: event.target.value
      //^bracket notation to access the value being passed in for field
    });
	}
